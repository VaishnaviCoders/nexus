...
import { ArrowRightIcon, PhoneCall, PlusIcon } from "lucide-react";
  ...
  return (
    <div className="relative mx-auto flex w-full max-w-3xl flex-col justify-between gap-y-4 border-y px-4 py-8 dark:bg-[radial-gradient(35%_80%_at_25%_0%,--theme(--color-foreground/.08),transparent)]">
      ...
      <p className="text-balance text-center font-medium text-muted-foreground text-sm md:text-base">
        Begin your 30-days free trial today to fully explore and experience all
        the features and benefits we offer.
      </p>

      <div className="flex items-center justify-center gap-2">
        <Button variant="outline" className="flex items-center space-x-2"><PhoneCall />Talk To Expert </Button>
        <Button asChild>
          <Link href={"/select-organization"}>Get Started<ArrowRightIcon /></Link>
        </Button>
      </div>
    </div>
  );
